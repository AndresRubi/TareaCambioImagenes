...
//            }
        ...
        else if(estado_teclas[SDL_SCANCODE_D])
        {
            rect_character.x++;
            if(colision(rect_character,rect_character_otro))
                rect_character.x--;
        }
        else  if(estado_teclas[SDL_SCANCODE_S])
        {
            rect_character.y++;
            if(colision(rect_character,rect_character_otro))
                rect_character.y--;
        }
        else if(estado_teclas[SDL_SCANCODE_A])
        {
            rect_character.x--;
            if(colision(rect_character,rect_character_otro))
                rect_character.x++;
        }else if(estado_teclas[SDL_SCANCODE_SPACE])
        {
            rect_character_otro.x=rect_character.x+30;
            rect_character_otro.y=rect_character.y+30;
            //rect_character.x--;
        }
        ...
        //cout<< ((double)frame)/(SDL_GetTicks()/1000)<< endl;
        e.dibujar();
        e.logica();
        SDL_RenderPresent(renderer);
        frame++;
//           cout<<SDL_GetTicks()/frame<<endl;
//           SDL_Delay(15);
